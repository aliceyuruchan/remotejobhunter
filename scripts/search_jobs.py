...
def search_greenhouse(source, keywords):
...
def search_lever(source, keywords):
...
def search_ats_sources(config):
    ...
    for source in config["search"].get("ats_sources", []):
        ...
        if source.get("type") == "greenhouse":
            all_jobs.extend(search_greenhouse(source, keywords))
        elif source.get("type") == "lever":
            all_jobs.extend(search_lever(source, keywords))
        time.sleep(0.5)
    return all_jobs
